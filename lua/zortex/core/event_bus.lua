-- core/event_bus.lua
-- Event system with priority handling and async execution
local M = {}
    ...
    local function run()
      ...
      if elapsed > opts.max_time then
        vim.notify(string.format(
          "[EventBus] Slow handler '%s' for event '%s': %.1fms", 
          handler_name, event, elapsed
        ), vim.log.levels.WARN)
      end
      ...
      if not ok then
        vim.notify(string.format(
          "[EventBus] Handler '%s' error for event '%s': %s",
          handler_name, event, err
...
function EventBus:emit(event, data, opts)
  ...
  if vim.g.zortex_debug_events then
    vim.notify(string.format(
      "[EventBus] Emit '%s' with %d handlers",
      event, #handlers
