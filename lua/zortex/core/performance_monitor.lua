-- core/performance_monitor.lua
-- Advanced performance monitoring for Zortex
local M = {}
...
local function generate_report()
    ...
    if stats.p95 > (thresholds[operation] or 50) * 2 then
      table.insert(report.recommendations, {
        ...
        message = string.format(
          ...
          operation, stats.p95, thresholds[operation] or 50
        ),
      })
    end
  end
  ...
  if #metrics.memory_usage > 0 then
    local memory_values = vim.tbl_map(function(s) return s.used_kb end, metrics.memory_usage)
    ...
    if #metrics.memory_usage > 2 then
      ...
      local duration = metrics.memory_usage[#metrics.memory_usage].timestamp - 
                      metrics.memory_usage[1].timestamp
...
function M.start()
  ...
  memory_timer = vim.fn.timer_start(
    sampling_config.memory_interval,
    function()
  ...
  report_timer = vim.fn.timer_start(
    sampling_config.report_interval,
    function()
      vim.schedule(function()
...
function M.show_report()
  ...
  table.insert(lines, string.format(
    ...
    "Operation", "Count", "Avg(ms)", "P50(ms)", "P95(ms)", "P99(ms)", "Slow", "Rate(%)"
  ))
  ...
  for _, op in ipairs(sorted_ops) do
    ...
    local line = string.format(
      ...
      op, stats.count, stats.avg, stats.p50, stats.p95, stats.p99,
      stats.slow_count, stats.slow_rate
    )
