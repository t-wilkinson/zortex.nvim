	...
	for i, line in ipairs(lines) do
		...
		if okr_date then
			...
			if created_date then
				-- Parse date
				local date_parsed = datetime.parse_date(created_date)
				if date_parsed then
					current_objective.created_date = os.time(date_parsed)
				end
			end
...
function M.update_okr_progress()
	...
	for _, objective in ipairs(objectives) do
		...
		local new_line = parser.update_attribute(old_line, "progress", string.format("%d/%d", completed_krs, total_krs))
		...
		if is_completed and not was_completed then
			...
			if #objective.area_links > 0 then
				...
				xp_notifs.notify_objective_completion(objective.title, xp_awarded, area_awards)
			end
