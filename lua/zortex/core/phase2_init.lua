...
function M.setup_dev_commands()
  ...
  vim.api.nvim_create_user_command("ZortexXPStats", function()
    ...
    for i, area in ipairs(top_areas) do
      table.insert(lines, string.format("  %d. %s - Level %d (%d XP)",
        i, area.path, area.level, area.xp))
    end
    ...
    for i, proj in ipairs(leaderboard) do
      ...
      table.insert(lines, string.format("  %d. %s %s - Level %d (%d XP)",
        i, status, proj.name, proj.level, proj.xp))
    end
  ...
  vim.api.nvim_create_user_command("ZortexMonitorXPEvents", function(opts)
    ...
    local event_names = {
      "task:completing", "task:completed", "task:uncompleted",
      "xp:calculated", "xp:awarded", "xp:distributed", "xp:reversed"
    }
