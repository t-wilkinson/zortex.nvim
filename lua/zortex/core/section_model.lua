-- core/section.lua
-- Section model - first-class representation of document structure
local M = {}
...
function Section:can_contain(other_section)
  return constants.SECTION_HIERARCHY.can_contain(
    self.type, self.level,
    other_section.type, other_section.level
  )
end
...
function SectionTreeBuilder:add_section(section)
  ...
  while #self.stack > 0 do
    ...
    if potential_parent:can_contain(section) and 
       section.start_line <= potential_parent.end_line then
...
function M.create_from_line(line, line_num)
  ...
  if section_type == constants.SECTION_TYPE.TEXT or 
     section_type == constants.SECTION_TYPE.TAG then
