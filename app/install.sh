...
get_latest_release() {
  fetch "https://api.github.com/repos/t-wilkinson/zortex.nvim/releases/latest" |
    grep '"tag_name":' |
    sed -E 's/.*"([^"]+)".*/\1/'
}
...
download() {
  mkdir -p bin
  cd bin
  url="https://github.com/t-wilkinson/zortex.nvim/releases/download/$tag/${1}"
...
arch=$(uname -sm)
case "${arch}" in
  "Linux x86_64") download zortex-linux.tar.gz ;;
  "Linux i686") download zortex-linux.tar.gz ;;
  "Darwin x86_64") download zortex-macos.tar.gz ;;
  "Darwin arm64") download zortex-macos.tar.gz ;;
