...
function M.show_dashboard()
	...
	table.insert(
		...
		string.format(
			...
			stats.tasks.total_tasks > 0 and (stats.tasks.completed_tasks / stats.tasks.total_tasks * 100) or 0
		)
	)
	...
	if stats.xp and stats.xp.season then
		...
		table.insert(lines, string.format("XP: %s", require("zortex.xp.core").format_xp(stats.xp.season.xp)))
